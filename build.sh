...
install -v -m644 "${SHED_PKG_CONTRIB_DIR}/hostname" "${SHED_FAKE_ROOT}${SHED_PKG_DEFAULTS_INSTALL_DIR}/etc" &&
install -v -m644 "${SHED_PKG_CONTRIB_DIR}/hosts" "${SHED_FAKE_ROOT}${SHED_PKG_DEFAULTS_INSTALL_DIR}/etc" &&
# Default automount config
install -v -m644 "${SHED_PKG_CONTRIB_DIR}/fstab" "${SHED_FAKE_ROOT}${SHED_PKG_DEFAULTS_INSTALL_DIR}/etc" &&
# Default login prompt config
install -v -m644 "${SHED_PKG_CONTRIB_DIR}/issue" "${SHED_FAKE_ROOT}${SHED_PKG_DEFAULTS_INSTALL_DIR}/etc" || exit 1
...
if [ -n "${SHED_PKG_LOCAL_OPTIONS[bootstrap]}" ]; then
        ...
        ln -sv /tools/lib/libstdc++.{a,so{,.6}} "${SHED_FAKE_ROOT}/usr/lib" || exit 1
        for SHED_PKG_LOCAL_LIB in blkid lzma mount uuid
        do
            ln -sv /tools/lib/lib${SHED_PKG_LOCAL_LIB}.so* "${SHED_FAKE_ROOT}/usr/lib" || exit 1
        done
        ...
        chgrp -v utmp "${SHED_FAKE_ROOT}/var/log/lastlog" || exit 1
fi
